...
pub use check::{CheckResult, CheckContext, run_all_checks, print_report,
    count_issues, has_fixable_issues, layer_status_report};
pub use config::{SootieConfig, load_config, generate_default_config, resolve_vision_model_path};
pub use model_download::{download_showui_model, update_config_with_model_path, estimate_model_size};
pub use python_env::{find_python, create_venv, install_deps, check_python_deps, fix_python_deps,
    sootie_venv_path, sootie_venv_python};
pub use sidecar::{launch_sidecar, install_sidecar_files, is_sidecar_running};
...
pub async fn run_setup(mode: SetupMode) -> Result<()> {
    ...
    if fails > 0 || warns > 0 {
        println!("{} issues found, {} optional features available.", 
            fails, warns);
    } else {
...
async fn fix_issue(result: &CheckResult, mode: SetupMode) -> Result<()> {
    match result.name {
        ...
        "Vision model + sidecar" => {
            ...
            let model_exists = model_path.is_some() && model_path.as_ref().unwrap().join("model.safetensors").exists();
            ...
            if !model_exists {
                ...
                let do_download = if mode == SetupMode::AutoFix {
                    ...
                    ask_yes_no(
                        format!("[Vision] Download ShowUI-2B model (~{}GB from Hugging Face)?", size_gb), 
                        false
                    )?
                };
            ...
            if !deps_ok {
                let deps_name = if platform == "macos" { "mlx-vlm, transformers" } else { "transformers, torch" };
                let do_deps = if mode == SetupMode::AutoFix {
                    true
                } else {
                    ask_yes_no(format!("[Vision] Install Python dependencies ({})?", deps_name), true)?
                };
