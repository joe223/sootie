...
impl PerceptionProvider for LinuxPerceptionProvider {
    async fn get_context(&self) -> Result<Context, PerceptionError> {
        Err(PerceptionError::NotImplemented(
            "Linux AT-SPI2 not yet implemented".to_string(),
        ))
    }
    ...
    async fn screenshot(
        ...
        Err(PerceptionError::NotImplemented(
            "Linux screenshot not yet implemented".to_string(),
        ))
    }
}

pub struct LinuxActionProvider;

impl LinuxActionProvider {
    pub fn new() -> Self {
        Self
    }
}
...
#[async_trait]
impl crate::action::ActionProvider for LinuxActionProvider {
    async fn click(
        &self,
        ...
        Err(crate::action::ActionError::NotImplemented(
            "Linux XTest not yet implemented".to_string(),
        ))
    }
